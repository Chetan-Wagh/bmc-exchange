...
#gk-header-mod .big-btn';
	    ...
	    $wp_customize->add_setting( 
	    	...
	    	array( 
	    		...
	    		'capability' => 'edit_theme_options',
	    		'sanitize_callback' => 'events_sanitize_blocks_color'
	    	)
	    );
		...
		$wp_customize->add_setting(
			...
			array(
			    'default'   => 'google',
			    'capability' => 'edit_theme_options',
			    'sanitize_callback' => 'events_sanitize_font'
			)
		);
		...
		$wp_customize->add_setting(
			...
			array( 
				'default'   => '1240',
				'capability' => 'edit_theme_options',
				'sanitize_callback' => 'events_sanitize_number'
			)
		);
		...
		$wp_customize->add_setting(
			...
			array( 
				'default'   => 'right',
				'capability' => 'edit_theme_options',
				'sanitize_callback' => 'events_sanitize_sidebar_pos'
			)
		);
		...
		$wp_customize->add_setting(
            ...
            array(
                'default'   => '0',
                'capability' => 'edit_theme_options',
                'sanitize_callback' => 'events_sanitize_switch'
            )
        );
		...
		$wp_customize->add_setting(
			...
			array( 
				'default'   => 'default',
				'capability' => 'edit_theme_options',
				'sanitize_callback' => 'events_sanitize_date_format'
			)
		);
		...
		$wp_customize->add_control(
		    ...
		    array(
		        ...
		        'active_callback' => 'events_show_font_body',
		        'type'     => 'text',
		        'priority' => 1
	    	)
		);
        ...
        $wp_customize->add_control(
            ...
            array(
                ...
                'active_callback' => 'events_show_font_headers',
                'type'     => 'text',
                'priority' => 4
            )
        );
        ...
        $wp_customize->add_control(
            ...
            array(
                ...
                'active_callback' => 'events_show_font_others',
                'type'     => 'text',
                'priority' => 7
            )
        );
...
function events_customizer_fonts($group, $font, $selectors) {
    ...
    if (get_theme_mod('events_'.$group.'_google_font', $font) !== '' && get_theme_mod('events_'.$group.'_font', 'google') == 'google') {
    	...
    	$font = esc_attr(get_theme_mod('events_'.$group.'_font', 'arial'));
    }
...
function events_show_font_body($control) {
    $option = $control->manager->get_setting('events_body_font') ;
    return $option->value() == 'google';
}

function events_show_font_headers($control) {
    $option = $control->manager->get_setting('events_headers_font') ;
    return $option->value() == 'google';
}

function events_show_font_others($control) {
    $option = $control->manager->get_setting('events_other_font') ;
    return $option->value() == 'google';
}

function events_sanitize_number($value) {
	if(is_numeric($value)) {
		return $value;
	}
...
function events_sanitize_switch($value) {
	if($value == '0' || $value == '1') {
		return $value;
	}
...
function events_sanitize_font($value) {
	$fonts = array(
		'google', 
		'verdana', 
		'georgia', 
...
function events_sanitize_sidebar_pos($value) {
...
function events_sanitize_blocks_color($value) {
	if($value === 'events-light-blocks' || $value === 'events-dark-blocks') {
		return $value;
	}
	return null;
}

function events_sanitize_date_format($value) {
